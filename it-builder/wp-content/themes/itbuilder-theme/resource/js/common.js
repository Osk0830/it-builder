...
$(function () {
  ...
  $(".js-accordionBtn").click(function () {
    $(this)
      .toggleClass("is-on")
      .parents(".js-accordionBox")
      .find(".js-accordionBoxInner")
      .slideToggle();
    return false;
  });
  ...
  $('a[href^="#"]:not([class*=js-])').click(function () {
    ...
    var topicpathH =
      $(".topicpath").length > 0 ? $(".topicpath").outerHeight() : 0;
    if ($(window).width() > tabWidth) {
...
$(function () {
  if ($(".js-sectionNav").length) {
    ...
    $(window).on("load scroll", function () {
      ...
      if (
        $($(".js-sectionNav a:last").attr("href")).next().length &&
        !$($(".js-sectionNav a:last").attr("href"))
          .next()
          .hasClass("js-popupBlock")
      ) {
        endBlock =
          $($(".js-sectionNav a:last").attr("href")).next().offset().top -
          $(window).height() / 2;
      }
    ...
    if ($dep_dl.find("input[type=checkbox]:checked").length > 0) {
      dep = $dep_dl
        ...
        .map(function () {
          ...
          return (class_name =
            $(this).attr("name").replace("[]", "_") + $(this).val());
        })
        .get();
    }
...
*/
  ...
  $search_button.on("click", function () {
    if ($(this).attr("id") === top_button) {
      search_post();
      $("html,body").animate(
        { scrollTop: $("#case-list").offset().top },
        500,
        "swing",
      );
    } else {
